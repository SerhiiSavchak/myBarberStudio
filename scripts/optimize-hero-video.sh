#!/bin/bash
# Optimize hero video: create WebM (smaller) and optionally compress MP4
# Requires: ffmpeg
# Run from project root: ./scripts/optimize-hero-video.sh

set -e
INPUT="public/hero-video.mp4"
WEBM="public/hero-video.webm"

if [ ! -f "$INPUT" ]; then
  echo "Error: $INPUT not found"
  exit 1
fi

echo "Creating WebM (VP9, no audio, 720p max)..."
ffmpeg -y -i "$INPUT" -c:v libvpx-vp9 -b:v 800k -maxrate 1.2M -bufsize 2M \
  -vf "scale=min(1280,iw):min(720,ih):force_original_aspect_ratio=decrease" \
  -an -loop 1 -t 12 "$WEBM"

echo "Done. WebM saved to $WEBM"
echo "Optional: compress MP4 with: ffmpeg -i $INPUT -c:v libx264 -crf 28 -preset slow -an -movflags +faststart public/hero-video-compressed.mp4"
