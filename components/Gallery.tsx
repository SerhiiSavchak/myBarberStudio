...
function BeforeAfterSlider() {
  ...
  return (
    ...
    >
      ...
      {/* Divider */}
      <div className="absolute top-0 bottom-0 z-10 w-px bg-foreground/50" style={{ left: `${pos}%` }}>
        <div className="absolute left-1/2 top-1/2 flex h-8 w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center border border-foreground/15 bg-background/60 backdrop-blur-sm">
          <svg viewBox="0 0 16 16" fill="none" stroke="hsl(var(--foreground))" strokeWidth="1.5" className="h-3.5 w-3.5 opacity-50">
            <path d="M5 3L2 8l3 5M11 3l3 5-3 5" />
          </svg>
        </div>
      </div>
      {/* Labels */}
      <span className="absolute left-4 bottom-4 z-10 bg-background/40 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.3em] text-foreground/60 backdrop-blur-sm">
        До
      </span>
      <span className="absolute right-4 bottom-4 z-10 bg-background/40 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.3em] text-foreground/60 backdrop-blur-sm">
        Після
      </span>
    </div>
  );
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div ref={ref} className="grid gap-px bg-border/50 sm:grid-cols-2 lg:grid-cols-3">
          {GALLERY_IMAGES.map((img, i) => (
            ...
            >
              ...
              {/* Corner marks */}
              <div className="absolute left-3 top-3 h-4 w-4 border-l border-t border-foreground/0 transition-all duration-500 group-hover:border-foreground/10" />
              <div className="absolute bottom-3 right-3 h-4 w-4 border-b border-r border-foreground/0 transition-all duration-500 group-hover:border-foreground/10" />
            </motion.div>
          ))}
        </div>
        ...
        <div className="mx-auto mt-16 max-w-3xl">
          <div className="mb-4 flex items-center gap-3">
            <span className="block h-px w-6 bg-neon-cyan/20" />
